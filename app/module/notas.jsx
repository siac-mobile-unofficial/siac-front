...
export default function Notes(){
    ...
    const Empty = ()=>{return(<View style={{padding:8}}><Text>Nenhuma nota registrada</Text></View>)}
    return(<SafeAreaView style={style.body}>
              <Header/>
               <View style={style.bodyView}>
                    <View style={style.bigView}>
                            <View style={style.infoView}>  
                            {!data?<ActivityIndicator color={theme.primaryColor} 
                            style={{flex:1,alignItems:"center",justifyContent:"center",width:"100%"}} 
                            size={64}/>
                            :<FlatList data={data}  
                            style={{padding:4}} 
                            ListHeaderComponent={()=>{return <View style={{padding:16,alignItems:"center",justifyContent:"center"}}>
                                <Text style={{fontSize:18,fontWeight:"600"}}>Notas</Text>
                                </View>}}
                            ItemSeparatorComponent={()=>{return <View style={{height:5}}></View>}}
                            ListEmptyComponent={Empty}
                            keyExtractor={(item,index)=>index.toString()}
                            renderItem={({item})=>{return <View style={{elevation:2,backgroundColor:theme.secondColor,padding:16,margin:5}}>
                                <Text>{item.name}</Text>
                                </View>}} />}
